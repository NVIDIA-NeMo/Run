...
class KubeflowExecutor(Executor):
    ...
    def code_dir(self) -> str:
        ...
        Path is ``<mountPath>/<username>/code``. When ``workdir_volume_mount`` is unset,
        ``mountPath`` defaults to ``/nemo_run`` for local planning only (no PVC sync).
        When ``workdir_volume_mount`` is set, ``mountPath`` must be present or a
        ``ValueError`` is raised.
        """
        return f"{self._code_mount_root().rstrip('/')}/{getpass.getuser()}/code"
