...
NeMo Run CLI is optimized for ML experiments with nested, type-safe, Python-first configuration. This section compares it to argparse, Hydra, and Typer, outlining configuration ergonomics, experiment execution features, and when to choose each tool.
...
Compare NeMo Run CLI feature-by-feature against argparse, Hydra, and Typer. The table highlights configuration ergonomics, file format support, executor integration, and learning curve to help you choose the right tool for your workflow.
...
Choose the CLI that aligns with your workflow by weighing configuration complexity, file format needs, Python-first ergonomics, and execution targets.
...
Get familiar with the core building blocks you’ll use to define commands, configure objects, and orchestrate experiments with the CLI.
...
Start with a minimal entrypoint, run it from the command line, and see how overrides, validation, and help output work in practice. Use these examples to confirm your setup and build from a simple script to more advanced workflows.
...
Enhance your setup with structured types, file-based inputs, and exports. Model nested settings with dataclasses, load YAML/TOML/JSON via the `@` syntax, and export the resolved configuration for reproducibility.
...
Define and register a DockerExecutor:
...
Define and register a SlurmExecutor:
