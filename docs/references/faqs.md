...
**A:** NeMo Run supports two execution modes for Slurm clusters:
...
executor = run.SlurmExecutor(
    ...
    nodes=2,
    gpus_per_node=4,
    tunnel=ssh_tunnel  # Use SSH tunnel for remote execution
)
```

**Local Execution (from the login node):**
```python
from nemo_run.core.execution.slurm import LocalTunnel

# Local tunnel when already on the cluster
...
executor = run.SlurmExecutor(
    ...
    nodes=2,
    gpus_per_node=4,
    tunnel=local_tunnel  # Use local tunnel when on login node
)
```

**Key Differences:**
- **SSHTunnel**: Use when executing from your local machine to access remote cluster
- **LocalTunnel**: Use when you're already logged into the cluster's login node
- **No tunnel**: Use when executing directly on compute nodes (not recommended)
...
This FAQ covers the most common questions about NeMo Run. For more detailed information, refer to the specific guides for [Configuration](../guides/configuration.md), [CLI Reference](cli-reference.md), [Execution](../guides/execution.md), and [Management](../guides/management.md).
