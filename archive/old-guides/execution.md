---
title: "NeMo Run Execution Guide"
description: "Comprehensive guide to executing NeMo Run tasks across different computing environments including local, Docker, Slurm, and cloud platforms"
categories: ["execution", "guides"]
tags: ["nemo-run", "execution", "executors", "slurm", "docker", "cloud", "kubernetes"]
...
- `torchrun` or `run.Torchrun`: This will launch the task using `torchrun`. Refer to the `Torchrun` class for configuration options. You can use it using `executor.launcher = "torchrun"` or `executor.launcher = Torchrun(...)`.
- `ft` or `run.core.execution.FaultTolerance`: This will launch the task using NVIDIA's fault tolerant launcher. Refer to the `FaultTolerance` class for configuration options. You can use it using `executor.launcher = "ft"` or `executor.launcher = FaultTolerance(...)`.

:::{note}
Launcher may not work very well with `run.Script`. Please report any issues at <https://github.com/NVIDIA-NeMo/Run/issues>.
:::

(old-guides-packagers)=
### Packagers

The packager support matrix is described as follows:
...
`run.GitArchivePackager` uses `git archive` to package your code. Refer to the API reference for `run.GitArchivePackager` to view the exact mechanics of packaging using `git archive`.
...
:::{dropdown} HybridPackager Example
:icon: code-square
...
(old-guides-defining-executors)=
### Defining Executors
Next, We'll describe details on setting up each of the executors.

(old-guides-local-executor)=
#### LocalExecutor
...
:::{dropdown} Docker Executor Configuration Example
:icon: code-square
...
:::{dropdown} Slurm Executor Configuration Example
:icon: code-square
...
(old-guides-job-dependencies)=
##### Job Dependencies

`SlurmExecutor` supports defining dependencies between jobs, allowing you to create workflows where jobs run in a specific order. Additionally, you can specify the `dependency_type` parameter:
...
:::{dropdown} SkypilotExecutor Configuration Example
:icon: code-square
...
:::{dropdown} DGXCloudExecutor Configuration Example
:icon: code-square
...
:::{dropdown} LeptonExecutor Configuration Example
:icon: code-square
