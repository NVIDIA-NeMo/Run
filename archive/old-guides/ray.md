---
title: "Ray Clusters & Jobs with NeMo Run"
description: "Comprehensive guide to using NeMo Run with Ray for distributed computing on Kubernetes and Slurm"
categories: ["ray", "distributed-computing", "guides"]
tags: ["nemo-run", "ray", "kubernetes", "slurm", "distributed", "clusters", "jobs"]
...
| Best for | Debugging, notebooks, iterative development, hyper-param sweeps that reuse workers | CI/CD pipelines, scheduled training/eval, one-off runs |
...
(old-guides-kuberay-quick-start)=
## 2.  KubeRay quick-start

:::{dropdown} KubeRay Configuration Example
:icon: code-square
...
(old-guides-kuberay-notes)=
### Notes
1. `workdir` is rsynced into the first declared `volume_mounts` on the executor, so relative imports *just work*.
...
(old-guides-slurm-quick-start)=
## 3.  Slurm quick-start

:::{dropdown} Slurm Ray Configuration Example
:icon: code-square
...
job.start(
    ...
    workdir="/path/to/project/",   # rsynced via SSH to the cluster_dir/code/
    pre_ray_start_commands=pre_ray_start,
)
job.logs(follow=True)
...
(old-guides-rolling-your-own-cli)=
